  ...
  <div class="max-w-screen-lg mx-auto p-4">
    ...
    >
      ...
      <div class="mb-2 flex flex-row items-center text-sm gap-0.5">
        ...
        <NuxtLink to="#" class="text-gray-600 hover:text-gray-950">
          <div class="flex items-center gap-0.5">
            <Icon name="ph:star-fill" class="text-base text-purple-500" />
            <span class="leading-relaxed">神奇糖果</span>
          </div>
        </NuxtLink>
      </div>
      <h2 class="text-2xl font-bold mt-4">社区奖励</h2>
      <div class="flex gap-5 mt-5 items-center">
        <img src="https://i.psray.net/i/2024/07/17/6696afa80d3c4.png" class="size-10" />
        <div>
          <h2 class="text-lg font-bold">神奇糖果</h2>
          <p class="text-sm text-gray-600">散发着温暖而甜美的香气糖果，吃下后整个人会充满元气</p>
        </div>
      </div>
      <div class="mt-5">
          ...
          <div v-for="(rankedUser, index) in data" class="flex items-center px-1 py-2">
            ...
            <div class="flex flex-1 justify-end gap-5">
              <div class="flex items-center gap-3">
                <img src="https://i.psray.net/i/2024/07/17/6696afa80d3c4.png" class="size-6" />
                <span class="w-10">{{ rankedUser.rare_candy }}</span>
              </div>
            </div>
          </div>
        </div>
...
const { data, error, refresh } = await useAsyncData('candy-ranking', fetchData, {
  watch: [rage],
});

async function fetchData() {
  const res = await api.get(`/laboratory/rewards/candy`, {
    params: {
