...
// `markdown.*` is owned by @psray/markdown-editor and merged in at build time, so those
// keys are legitimately usable from app/ without appearing in our own catalogs.
const moduleLocalesDir = join(root, 'node_modules/@psray/markdown-editor/dist/runtime/locales')
...
/** Keys a template may reference: ours, plus anything a module contributes. */
const provided = new Set(source)
const moduleCatalog = join(moduleLocalesDir, `${SOURCE_LOCALE}.json`)
if (existsSync(moduleCatalog)) {
  flatten(JSON.parse(readFileSync(moduleCatalog, 'utf8')), '', provided)
}
...
for (const [key, file] of [...used].sort()) {
  if (!provided.has(key)) problems.push(`${file} uses undefined key  ${key}`)
}
